...
const items: { label: string; icon: LucideIcon }[] = [
  { label: "Licensed & Bonded", icon: Shield },
  { label: "20+ Years Experience", icon: Clock },
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-12 md:gap-4 md:divide-x divide-border-subtle">
          {items.map(({ label, icon: Icon }, i) => (
            ...
            >
              ...
              <h4 className="text-foreground-secondary font-serif text-2xl font-medium tracking-wider">{label}</h4>
            </motion.div>
          ))}
        </div>
      </div>
